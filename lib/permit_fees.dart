...
// First, check all floors in costPricingTable are in permitFeeFloorAreas and match area
        if (permitFeeFloorAreas.isNotEmpty) {
            ...
            for (final permitFloor in permitFeeFloorAreas) {
              if (floorNumber == permitFloor[0]) {
                ...
                if (floorArea != permitFloor[1]) {
                  differentFloors.add(floorNumber);
                }
                break;
              }
            }
          ...
          for (final permitFloor in permitFeeFloorAreas) {
            ...
            if (!found) {
              differentFloors.add(permitFloorNumber);
            }
          }
        }


/*  if (differentFloors.isNotEmpty) {
    print('Floors with different or missing permit fee areas:');
    for (final floor in differentFloors) {
      print(' - Floor $floor');
    }
  }
  else {
    print('All floor areas match the permit fee data.');
    for (final floor in permitFeeFloorAreas) {
      print(' - Floor ${floor[1]}');
    }
  }*/
  ...
  return {
    ...
    'differentFloors': differentFloors,
  };
...
class _FloorRangesPageState extends State<FloorRangesPage> {
  ...
  Future<void> getFloorRangesAndMaxFloorData()
    ...
    final differentFloors = result['differentFloors'];
    ...
    setState(() {
      ...
      differentFloorsData = differentFloors;
      maxFloorNumber = maxFloor;
      differentFloorsBool = differentFloorsData.isNotEmpty;
    });
  }
...
// iPad sizes (larger)
 ...
 //   final double spacingHeight = isIpad ? 16.0 : 10.0;
    ...
    return Scaffold(
      ...
      body: Container(
        color: const Color.fromRGBO(86, 2, 7, 1.0), // Add a background color to the entire page
        child: SafeArea(
          child: Column(
            children: [

              Visibility(
                visible: differentFloorsBool,
                child: Padding(
                      ...
                      //      '  for the following floors. '
                            ...
                            // TextSpan(text: differentFloorsData.join(', ')),
                          ],
                        ),
                      ),
                    ),
              ...
              Row(
                children: [
                  ...
                  Expanded(flex:5,
                    child: Text(
                      'Please set the permit fee for all the following floors on the next page. ',
                      style: TextStyle(
              ...
              Expanded(
                child: SingleChildScrollView(
                  child: Container(
                    ...
                    child: SizedBox(
                      height:  MediaQuery.of(context).size.height * .6, // Fixed maximum height for the table
                      child: SingleChildScrollView(
                            ...
                            showDialog(
                              ...
                              builder: (BuildContext context) {
                                return AlertDialog(
                                  ...
                                  content:  Text('\nIn the next page, determine the permit fee for each '
                                      'floor exactly based on the area shown in the table. The floors'
                                      ' and their corresponding areas have'
                                      ' been defined in the previous parts. So, go to the next page and '
                          ...
                          if (!differentFloorsBool)
                          {
                            NavigationService().navigateToScreen(
                              PermitFeeInputs(
                                ...
                                givenMaxFloor: maxFloorNumber,
                       //         firstStartingFloorForPermitFee: startingFloor,
                              ),
                              arguments: {
                                'givenProjectName': widget.givenProjectName,
                                //      'givenMaxConstructionValue': widget.givenMaxConstructionValue,
...
//    givenMaxConstructionValue = widget.givenMaxConstructionValue;
    ...
    startingFloor = Provider.of<ProjectData>(context, listen: false).firstStartingFloor;
    maxFloorInPermitFee = widget.givenMaxFloor;
...
// is used to prevent the check PermitFeeData method from rebuilding when the projectData changes.
  ...
  void checkPermitFeeData() async {
    ...
    if (hasData) { // by pressing edit project if the given project name isn't _oozz comes up
      _onNextPermitFeePlan(widget.givenProjectName, 1);
    }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content:  Text("Input cannot be empty and should be a valid number "
...
/*
  void saveCurrentSegmentOfFeeTableStartingFloor(int segmentNumber2)
  async { // saving data with same segment numbers, this Happens just for the row that has a setting icon
    ...
    { print('data.is not null ');
      await CompleteCalculationDatabaseHelper.insertOrUpdatePermitFeeStartingSimilarPercentageData(
        ...
        permitFeeStartingSimilarTableStartingFloor:  startingFloor,
        permitFeeStartingSimilarTableFeePlanNumber:permitFeePlanValue,
  //      permitFeeStartingSimilarTableFloorNumber: int.parse(permitFeeTableData[segmentNumber2 - 1].name.split(' ')[1]),
        permitFeeStartingSimilarTableSimilarFloor: feeSimilarFloorController.text.isNotEmpty ?
        permitFeeNumberOfSimilarFloorsSaved
            :  data.permitFeeStartingSimilarTableSimilarFloor,
        permitFeeStartingSimilarTableNumberOfSegments: numberOfSegmentsController.text.isNotEmpty ?
        numberOfFeeSegmentsSaved
            : data.permitFeeStartingSimilarTableNumberOfSegments,
        permitFeeStartingSimilarTableFeePercentage: _permitFeePercentageSelected &&
            _permitFeePercentageController.text.isNotEmpty ?
        double.parse(_permitFeePercentageController.text) :
        data.permitFeeStartingSimilarTableFeePercentage,
        permitFeeStartingSimilarTableFeePerMeter: _permitFeePerMeterSelected &&
            _permitFeePerMeterController.text.isNotEmpty
            ? double.parse(_permitFeePerMeterController.text) :
        data.permitFeeStartingSimilarTableFeePerMeter,
      )); print('permitFeeStartingSimilarTableFeePerMeter '
          '${data.permitFeeStartingSimilarTableFeePerMeter}');
      print('data.oopermitFee '
          '${data.permitFeeStartingSimilarTableFeePercentage}');
    }
    else {
      print('data.is null ');
    ...
    //    permitFeeStartingSimilarTableFloorNumber: int.parse(permitFeeTableData[segmentNumber2 - 1].name.split(' ')[1]),
        ...
        permitFeeStartingSimilarTableNumberOfSegments: numberOfSegmentsController.text.isNotEmpty
            ? numberOfFeeSegmentsSaved
            : -4321,
        permitFeeStartingSimilarTableFeePercentage: _permitFeePercentageSelected && _permitFeePercentageController.text.isNotEmpty ?
        double.parse(_permitFeePercentageController.text) : -4321,
        permitFeeStartingSimilarTableFeePerMeter: _permitFeePerMeterSelected && _permitFeePerMeterController.text.isNotEmpty ?
    ...
    for (int i = 0; i < permitFeeTableData.length; i++)
    {
      await CompleteCalculationDatabaseHelper.insertOrUpdatePermitFeeSegmentPricingData(PermitFeeSegmentPricingData(
        ...
        permitFeeSegmentPricingTableFeePlanNumber: permitFeePlanValue,
        permitFeeSegmentPricingTableSegmentArea: permitFeeTableData[i].textField2Controller.text.isNotEmpty
            ? double.parse(permitFeeTableData[i].textField2Controller.text)
            : -4321,
        permitFeeSegmentPricingTableSegmentFeePerMeter: permitFeeTableData[i].textField3Controller.text.isNotEmpty
            ? double.parse(permitFeeTableData[i].textField3Controller.text)
            : -4321,
        permitFeeSegmentPricingTableTotalSegmentPermitFee: (permitFeeTableData[i].textField3Controller.text.isNotEmpty &&
            permitFeeTableData[i].textField2Controller.text.isNotEmpty) ?
        double.parse(permitFeeTableData[i].textField3Controller.text) *
            double.parse(permitFeeTableData[i].textField2Controller.text)
            : -4321,
 ...
 /* Future<bool> _onNextCPP(String projectName, int cppValue)
    ...
    for (int i = 0; i < projectStartingSimilarData.startingSimilarTableNumberOfSegments; i++) {
      ...
      areaTableData.add(AreaTableRowData(name: rowName, textField1Controller: textField1Controller));
    }
...
/*  Future<void> profitCalculationForEachSegment() async {
  ...
  Future<bool> _onNextPermitFeePlan(String projectName, int permitFeePlanValue)
  async {
    ...
    numberOfSegmentsController.text =
        projectStartingSimilarData!.permitFeeStartingSimilarTableNumberOfSegments.toString();
    numberOfFeeSegmentsSaved = projectStartingSimilarData.permitFeeStartingSimilarTableNumberOfSegments;

    // Update the area table data
    segmentAreaTableData.clear();
    for (int i = 0; i < projectStartingSimilarData.permitFeeStartingSimilarTableNumberOfSegments; i++) {
      String rowName = 'Floor ${projectStartingSimilarData.permitFeeStartingSimilarTableStartingFloor.toString()} Fee Segment ${i + 1}';
    ...
    List<String> segmentNumbers = [];
    for (int i = 0; i < projectStartingSimilarData.permitFeeStartingSimilarTableSimilarFloor+1; i++)
    { for (int j = 0; j < projectStartingSimilarData.permitFeeStartingSimilarTableNumberOfSegments; j++) {
      segmentNumbers.add('Floor ${(projectStartingSimilarData.permitFeeStartingSimilarTableStartingFloor+i).toString()} '
          'Fee Segment ${j + 1}');
    }}
    ...
    for (int i = 0; i < retrievedPermitFeeTableData.length; i++) {
      String segmentNumber = segmentNumbers[i];
      TextEditingController textField2Controller = TextEditingController(text:
            retrievedPermitFeeTableData[i].permitFeeSegmentPricingTableSegmentArea.toString());
      TextEditingController textField3Controller = TextEditingController(text:
            retrievedPermitFeeTableData[i].permitFeeSegmentPricingTableSegmentFeePerMeter.toString());
      feeSimilarFloorController.text = projectStartingSimilarData.permitFeeStartingSimilarTableSimilarFloor.toString();
      permitFeeNumberOfSimilarFloorsSaved = projectStartingSimilarData.permitFeeStartingSimilarTableSimilarFloor;
      startingFloor= projectStartingSimilarData.permitFeeStartingSimilarTableStartingFloor;
      areaOfStartingFloor = getAreaForStartingFloor(startingFloor);
    ...
    if (( allFieldsAreNotEmpty &&
      ...
      await permitFeeCalculationForEachSegment().then((_) {
        ...
        if (permitFeePlanValue > 0) {
          ...
          _onNextPermitFeePlan(projectName1, permitFeePlanValue);
          setState(() {});
  ...
  Future<bool> checkFloorAreaRange(BuildContext context, List<List<dynamic>> floorRangesData,
      int maxFloor, int startingFloor, int permitFeeSimilarFloor, double areaOfFeePlan)
  async {
    bool isPossible = true;
    final screenWidth = MediaQuery.of(context).size.width;
    const ipadBreakpoint = 850.0;
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content: SingleChildScrollView( // Wrap the content in SingleChildScrollView
                child: Text(
                  'It is not possible to have floor $i, because the biggest number of '
                      'floors defined in the previous part is $maxFloor.'
                    ...
                    'For example, when a project has 3 basement floors and 5 above-ground floors, '
                              ...
                              'becomes floor 0, and the 5th above-ground floor becomes floor 4.'
                              ' Therefore, in total, 8 floors are constructed. If, in this example,'
                              ' you consider the starting floor as floor 1 instead of the -3rd floor, '
                              'then the ground floor becomes floor 4, and the top floor becomes floor 8.'
                              ...
                              ' buildings that have 8 above-ground floors. If the project does not have a basement,'
                              ' it is better to consider the ground floor as floor 0, because although shared'
                              ' floors such as parking may sometimes be above the ground floor, usually, '
                              'if the project does not have a basement, only the ground floor is the parking floor,'
                              ' and the upper floors have residential, commercial, or office uses that are sellable.'
                              ' For example, if we consider the ground floor as floor 0, when the top floor of'
                              ' a project is floor 6, it means there are 6 sellable floors above the parking,'
                              ' even though 7 floors (commonly referred to as 7 ceilings) have been constructed.',
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: Text('\nMake sure that the area for each floor number '
                  ...
                  'If there are any floors with different areas, they must belong to'
                    ...
                    'If there is just '
                    'one floor in the fee plan you should enter zero as number of similar floors.',
                  style: TextStyle(
                      ...
                      child: Padding( padding: const EdgeInsets.fromLTRB(8,2,8,0),
                        /*  padding: EdgeInsets.only(
                         ...
                         const SizedBox(height: 5,),
                             ...
                             Container(
                              color: const Color(0xFF1877C5), // Set the background color of the container to purple
                              child: Row(
                                children: [
                        ...
                        Container(color: const Color(0XFF00BCD4),
                          child: Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: Row(
                              children: [
                                        ...
                                        async {
                                          ...
                                          if (numberOfSegmentsController.text.isNotEmpty &&
                                            ...
                                            if (numberOfFeeSegmentsSaved > 10)
                                            {
                                              ...
                                              {
                                                segmentAreaTableData.clear();
                                                checkPermitFeeVisibility();
                                                for (int i = 0; i < numberOfFeeSegmentsSaved; i++) {
                                                  String segmentNumber = 'Floor $startingFloor Fee Segment ${i + 1}';
                                                  segmentAreaTableData.add(SegmentAreaTableRowData(
                                                    name: segmentNumber,
                                  ...
                                  Padding(
                                    ...
                                    child: Column(
                                      children: [
                                        Row(
                                          children: [
                                            (!(maxFloorInPermitFee == (startingFloor )) )?
                                              ...
                                              child: Expanded(
                                                ...
                                                child: ElevatedButton(
                                                 ...
                                                 //   int numberOfSegments = int.tryParse(numberOfSegmentsController.text) ?? 0;
                                                    ...
                                                    if ((!(maxFloorInPermitFee == (startingFloor )) ) &&
                                                        (feeSimilarFloorController.text.isEmpty ||
                                                            !isValidNumber(feeSimilarFloorController.text)) )
                                                    {
                                                      ...
                                                      permitFeeNumberOfSimilarFloorsSaved = int.tryParse(feeSimilarFloorController.text) ?? 0;
                                                      for (int i = 0; i < permitFeeNumberOfSimilarFloorsSaved + 1; i++) {
                                                        for (int j = 0; j < numberOfFeeSegmentsSaved; j++) {
                                                          segmentName.add(
                                                              'Floor ${startingFloor + i} Fee Segment ${j + 1}');
                                                        }
                                                      }
                                                        ...
                                                        for (int i = 0; i < segmentAreaTableData.length; i++) {
                                                          sumOfSegmentAreas += double.parse(
                                                              segmentAreaTableData[i].textField1Controller.text);
                                                        }
                                                          ...
                                                          showDialog(
                                                            ...
                                                            builder: (BuildContext context) {
                                                              return AlertDialog(
                                                                ...
                                                                content: Text('The total area of the segments does not equal '
                                                        ...
                                                        if (isValid) {
                                                          ...
                                                          permitFeeTableData.clear();
                                                          await checkPermitFeeVisibility();
                                                          feeSimilarFloor = feeSimilarFloorController.text.isEmpty ? 0 :
                                        ...
                                        child: SingleChildScrollView(
                                            ...
                                            columns:  [
                                               ...
                                               DataColumn(label: Text('Permit Fee \n(ft²/m²)'
                                                   ,style: TextStyle(color: Colors.white,
                                                     fontSize: textFontSize ,))),
                                              DataColumn(
                                                label: (permitFeeTableData.isNotEmpty && permitFeeTableData.last.name.split(' ')[1] ==
                                                    startingFloor.toString() && (permitFeeNumberOfSimilarFloorsSaved) > 0)
                                                    ?  Text(''
                                                ...
                                                cells: [
                                                  ...
                                                  DataCell(IconButton(
                                                      ...
                                                      onPressed: () {
                                                        ...
                                                        String segmentNumber = row.name.split(' ')[4];
                                                        String numberOfSegments_ = numberOfSegmentsController.text;
                
                                                        if (rowName.isNotEmpty && numberOfSegments_.isNotEmpty && segmentNumber.isNotEmpty ) {
                                                          int floor = int.parse(row.name.split(' ')[1]);
                                                          int segmentNumberInt = int.parse(segmentNumber);
                                                          int numberOfSegments = int.parse(numberOfSegments_);
                                          ...
                                          DataTable(
                                            ...
                                            columns:  [
                                              ...
                                              DataColumn(
                                                label: (permitFeeTableData.isNotEmpty && permitFeeTableData.last.name.split(' ')[1] ==
                                                    startingFloor.toString() && (int.tryParse(feeSimilarFloorController.text) ?? 0) > 0)
                                                    ?  Text(''
                                                    ,style: TextStyle( fontSize: textFontSize  * 0.8,))
                                                    :  const Text(''),
                                              ),
                                                ...
                                                cells: [
                                                  ...
                                                  DataCell(IconButton(
                                                     ...
                                                     //   int numberOfSegments = int.parse(numberOfSegments_);
                                                        saveCurrentSegmentOfFeeTableStartingFloor(segmentNumberInt);
                                                        permitFeeIconButtonFunction(context, floor, segmentNumberInt
                                                            , permitFeeNumberOfSimilarFloorsSaved,  startingFloor,
                                                            numberOfFeeSegmentsSaved);
                  ...
                  child: LayoutBuilder (
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () async {
                               ...
                               //       'firstStartingFloorForFloorRangesPage': startingFloor,
                                 ...
                                 /*    NavigationService().navigateToScreen(
                                    TableOfSegmentInputs(
                                      givenProjectName: projectName1,
                                      // we send firstStartingFloor from here to not call getStarting... to retrieve it in
                                      firstStartingFloor: startingFloor,
                                      givenCppValue: 1,
                                    ),
                                  );
                                 ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text('You are in the first permit fee section. '
                                          'Press the Home button to go to the Basic Data page.'),
                                    ),
                                  );*/
                                }
                              },
                            ...
                            IconButton(
                                ...
                                async {
                                          ...
                                          showDialog(
                                            ...
                                            builder: (BuildContext context) {
                                              return AlertDialog(
                                                ...
                                                content:  Text('Please fill in all text fields in the permit fee table.\n\n'
                                                    "(digits and optional decimal point only, like: 123, 123.5, 0.66) "
                                                    "not including letters (e.g., a, b, c) or symbols (e.g., \$, %, &)."
                                                    " Also starting or trailing decimal point (e.g., .1 or 1.) is not allowed.",
                                                  style: TextStyle(
                                    ...
                                    {// maxFloorInPermitFee Is calculated in Floor ranges page
                                  if (((startingFloor + permitFeeNumberOfSimilarFloorsSaved) < maxFloorInPermitFee)
                                      && feeTableVisible)
                                  {
                                ...
                                //    areaOfStartingFloor = getAreaForStartingFloor(startingFloor);
                                    await checkPermitFeeVisibility();
                                 // if checkPermitFeeVisibility says still a FeePlan exist to be retrieved, so onnext
                                    if (checkMaxFeePlanNumber) {
                                      _onNextPermitFeePlan(projectName1, permitFeePlanValue);
                                      setState(() {});
                                    }
                                    else {
                                      // If checkMaxFeePlanNumber is false, execute this code block
                                      segmentAreaTableData.clear();
                                 ...
                                 //     await checkPermitFeeVisibility();
                                      ...
                                      // Save current price table and rerun the page with no permitFeeTableData
                                      // priceTables.add(List.from(permitFeeTableData));
                                      // permitFeeTableData.clear();
                
                                      setState(() {
                                        startingFloor = startingFloor + (permitFeeNumberOfSimilarFloorsSaved) + 1;
                                // Don't comment the following code line since it is needed for updating area of starting floor
                                        areaOfStartingFloor = getAreaForStartingFloor(startingFloor);
                                      });
                                    }
                                  ...
                                  ( feeTableVisible && (maxFloorInPermitFee ==
                                    (startingFloor + permitFeeNumberOfSimilarFloorsSaved)) )

                                   {
                                      ...
                                      NavigationService().navigateToScreen(
                                        OtherCosts(
                                          ...
                                          givenMaxFloorNumber: maxFloorInPermitFee,
                   //                       givenFirstStartingFloorForPermitFee: startingFloor,
                      ...
                      //                    'givenFirstStartingFloorForPermitFee': startingFloor,
                          ...
                          /*  Visibility(
                              visible: (feeTableVisible && (maxFloorInPermitFee ==
                                              ...
                                              TextSpan(
                                               ...
                                               TextSpan(
                                                text: '\nIn this app, each "permit fee segment" represents an area of a floor '
                                                    ...
                                                    'to correspond to individual properties for sale. For instance, '
                                                    ...
                                                    'and the cost-price segments on each floor. For instance, one floor '
...
class FeeChangeRateDialogState extends State<FeeChangeRateDialog> {
  ...
  async {
    ...
    if (data != null ) {
      ...
      }  else if (data.permitFeeStartingSimilarTableFeePercentage == 0 && data.permitFeeStartingSimilarTableFeePerMeter == 0)
      { _permitFeePerMeterController.text = "";
      _permitFeePercentageController.text = "";
      setState(() {
        _permitFeePerMeterSelected = false;
